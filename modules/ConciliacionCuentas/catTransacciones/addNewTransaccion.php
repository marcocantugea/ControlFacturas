...
if(!empty($_POST)){
    if(isset($_POST['token']) && isset($_POST['descripcion']) && isset($_POST['active'])){
        $token=$_POST['token'];
        if($token==$SessionUser->token){
            $CatTrans= new TipoTransaccion();
            $CatTrans->descripcion=$_POST['descripcion'];
            $CatTrans->active=$_POST['active'];
            $_ADOTipoTransaccion = new ADOTipoTransaccion();
            $_ADOTipoTransaccion->AddNew($CatTrans);
